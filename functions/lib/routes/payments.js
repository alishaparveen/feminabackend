...
const createPaymentIntentSchema = zod_1.z.object({
    ...
    paymentType: zod_1.z.enum(['consultation', 'marketplace', 'subscription']),
    itemId: zod_1.z.string(), // booking ID, product ID, etc.
...
const refundSchema = zod_1.z.object({
    paymentIntentId: zod_1.z.string(),
    amount: zod_1.z.number().optional(), // Partial refund amount
    reason: zod_1.z.enum(['requested_by_customer', 'duplicate', 'fraudulent', 'subscription_canceled']).optional()
});
/**
 * POST /api/payments/create-intent
 * Create a payment intent for Stripe
 */
router.post('/create-intent', (0, validation_1.validateRequest)(createPaymentIntentSchema), async (req, res) => {
    try {
 ...
 */
router.post('/confirm', (0, validation_1.validateRequest)(confirmPaymentSchema), async (req, res) => {
    try {
 ...
 */
router.post('/create-subscription', (0, validation_1.validateRequest)(createSubscriptionSchema), async (req, res) => {
    try {
 ...
 */
router.post('/refund', (0, validation_1.validateRequest)(refundSchema), async (req, res) => {
    try {
...
// Helper functions
async function validatePaymentItem(paymentType, itemId, userId) {
    try {
        switch (paymentType) {
            case 'consultation':
...
async function checkRefundEligibility(paymentData) {
    try {
...
async function processRefund(paymentData, refundAmount) {
    try {
        switch (paymentData.paymentType) {
            case 'consultation':
