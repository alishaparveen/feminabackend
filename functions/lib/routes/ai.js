...
const chatMessageSchema = zod_1.z.object({
    ...
    category: zod_1.z.enum(['general', 'health', 'career', 'relationships', 'parenting', 'fitness']).optional(),
    context: zod_1.z.string().optional()
});
const feedbackSchema = zod_1.z.object({
    messageId: zod_1.z.string(),
    feedback: zod_1.z.enum(['helpful', 'not_helpful', 'inappropriate']),
 ...
 */
router.post('/chat', (0, validation_1.validateRequest)(chatMessageSchema), async (req, res) => {
    try {
        ...
        let contextPrompt = SYSTEM_PROMPTS[category];
        if (context) {
 ...
 */
router.post('/feedback', (0, validation_1.validateRequest)(feedbackSchema), async (req, res) => {
    try {
