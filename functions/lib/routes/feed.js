...
// Validation schemas
const feedFiltersSchema = zod_1.z.object({
    categories: zod_1.z.array(zod_1.z.string()).optional(),
    tags: zod_1.z.array(zod_1.z.string()).optional(),
    expertContent: zod_1.z.boolean().optional(),
    timeframe: zod_1.z.enum(['24h', '7d', '30d', 'all']).default('7d'),
    sortBy: zod_1.z.enum(['recent', 'popular', 'trending', 'recommended']).default('recommended')
});
const updatePreferencesSchema = zod_1.z.object({
    interests: zod_1.z.array(zod_1.z.string()),
    categories: zod_1.z.array(zod_1.z.string()),
    expertFollowing: zod_1.z.array(zod_1.z.string()).optional(),
    notificationSettings: zod_1.z.object({
        newPosts: zod_1.z.boolean().default(true),
        expertContent: zod_1.z.boolean().default(true),
...
router.get('/personal', async (req, res) => {
    try {
        ...
        const { page = 1, limit = 20, categories, tags, expertContent, timeframe = '7d', sortBy = 'recommended' } = req.query;
        ...
        if (tags) {
            ...
            posts = posts.filter(post => post.tags && post.tags.some((tag) => tagArray.includes(tag)));
        }
        ...
        switch (sortBy) {
            case 'recent':
                scoredPosts.sort((a, b) => b.createdAt.toDate().getTime() - a.createdAt.toDate().getTime());
                break;
            case 'popular':
                scoredPosts.sort((a, b) => (b.likes + b.comments * 2) - (a.likes + a.comments * 2));
                break;
            case 'trending':
        ...
        const authorIds = [...new Set(finalPosts.map(post => post.authorId))];
...
router.get('/trending', async (req, res) => {
    try {
        const { page = 1, limit = 20, timeframe = '7d', category } = req.query;
        // Calculate timeframe
        const timeframeDays = timeframe === '24h' ? 1 :
            timeframe === '7d' ? 7 :
                timeframe === '30d' ? 30 : 7;
        ...
        const authorIds = [...new Set(paginatedPosts.map(post => post.authorId))];
        const authorsSnapshot = await db.collection('users')
...
router.get('/experts', async (req, res) => {
    try {
        ...
        const enrichedPosts = posts.map(post => ({
            ...post,
            author: expertsMap.get(post.authorId) || null
        }));
        res.json({
            success: true,
            data: {
 ...
 */
router.put('/preferences', (0, validation_1.validateRequest)(updatePreferencesSchema), async (req, res) => {
    try {
        const userId = req.user.uid;
...
router.get('/suggestions', async (req, res) => {
    try {
        ...
        recentLikes.docs.forEach(doc => {
            ...
            if (post.category) {
                activityData.likedCategories.set(post.category, (activityData.likedCategories.get(post.category) || 0) + 1);
            }
            ...
            if (post.authorId) {
                activityData.interactedAuthors.set(post.authorId, (activityData.interactedAuthors.get(post.authorId) || 0) + 1);
            }
        });
...
// Helper functions
function calculatePersonalizationScore(post, userInterests, followedExperts, userData) {
    let score = 0;
...
async function updateFeedCache(userId, posts) {
    try {
...
async function generateExpertSuggestions(userId, activityData, userData) {
...
async function generateContentSuggestions(userId, activityData, userData) {
    const suggestions = [];
    // Generate content suggestions based on user activity
    return suggestions;
}
async function generateCategorySuggestions(userId, activityData, userData) {
    const suggestions = [];
