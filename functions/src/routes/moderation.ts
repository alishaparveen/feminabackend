...
const reportSchema = z.object({
  contentId: z.string(),
  contentType: z.enum(['post', 'comment', 'message', 'profile', 'product']),
  reason: z.enum(['spam', 'harassment', 'inappropriate', 'misinformation', 'hate_speech', 'violence', 'other']),
  description: z.string().min(10).max(500).optional(),
  evidence: z.array(z.string()).optional() // URLs to screenshots or evidence
});

const reviewSchema = z.object({
  action: z.enum(['approve', 'remove', 'warn', 'suspend_user', 'escalate']),
  reason: z.string().min(5).max(200),
  duration: z.number().optional(), // For suspensions in days
 ...
 */
router.post('/report', validateRequest(reportSchema), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    let automatedAnalysis: any = null;
    if (contentData) {
      try {
 ...
 */
router.put('/review/:reportId', validateRequest(reviewSchema), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { reportId } = req.params;
...
async function analyzeContentAutomatically(content: any, contentType: string): Promise<any> {
  try {
...
function extractTextFromContent(content: any, contentType: string): string {
  switch (contentType) {
  case 'post':
...
function calculatePriority(reason: string, automatedAnalysis: any): string {
...
function getCollectionName(contentType: string): string {
  const mapping = {
    post: 'posts',
    comment: 'comments',
    profile: 'users',
...
async function flagContentForReview(contentId: string, contentType: string, reportId: string): Promise<void> {
  try {
    const contentRef = db.collection(getCollectionName(contentType)).doc(contentId);
...
async function notifyAdministrators(reportData: any): Promise<void> {
