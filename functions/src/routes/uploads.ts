...
const bucket = getStorage().bucket(process.env.FIREBASE_STORAGE_BUCKET || 'femina-aee4b.appspot.com');
...
const uploadMetadataSchema = z.object({
  purpose: z.enum(['profile_avatar', 'post_image', 'product_image', 'document', 'verification']),
 ...
 */
router.post('/document', upload.single('document'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        error: 'No File',
 ...
 */
router.post('/multiple', upload.array('files', 5), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const batchPromises = successfulUploads.map(async (upload: any) => {
    ...
    // Update quota
    const successfulSize = successfulUploads.reduce((sum, upload: any) => sum + (upload.size || 0), 0);
 ...
 */
router.get('/my-files', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.uid;
    const { 
      type, 
    ...
    let query: any = db.collection('uploads').where('userId', '==', userId);
    ...
    // Apply sorting
    query = query.orderBy(sortBy.toString(), sortOrder as 'desc' | 'asc');
  ...
  try {
    ...
    try {
      if (fileData.type === 'image' && fileData.sizes) {
        ...
        const deletePromises = fileData.sizes.map((size: any) => 
          bucket.file(size.path).delete().catch(err => console.warn('File not found in storage:', size.path))
        );
...
async function processImage(buffer: Buffer, purpose: string): Promise<Array<{buffer: Buffer, suffix?: string, width?: number, height?: number}>> {
  const processed: Array<{buffer: Buffer, suffix?: string, width?: number, height?: number}> = [];
  ...
  try {
    ...
    case 'profile_avatar':
      ...
      processed.push({
        buffer: await sharp(buffer).resize(200, 200).jpeg({ quality: 90 }).toBuffer(),
        suffix: 'large',
        width: 200,
        height: 200
      });
      processed.push({
        buffer: await sharp(buffer).resize(100, 100).jpeg({ quality: 85 }).toBuffer(),
        suffix: 'medium',
        width: 100,
        height: 100
      });
      processed.push({
        buffer: await sharp(buffer).resize(50, 50).jpeg({ quality: 80 }).toBuffer(),
        suffix: 'small',
        width: 50,
        height: 50
      });
    ...
    case 'post_image':
      ...
      if (metadata.width && metadata.width > 1200) {
        processed.push({
          buffer: await sharp(buffer).resize(1200, null, { withoutEnlargement: true }).jpeg({ quality: 85 }).toBuffer(),
          suffix: 'large',
          width: 1200
        });
      }
      if (metadata.width && metadata.width > 800) {
        processed.push({
          buffer: await sharp(buffer).resize(800, null, { withoutEnlargement: true }).jpeg({ quality: 80 }).toBuffer(),
          suffix: 'medium',
          width: 800
        });
      }
      processed.push({
        buffer: await sharp(buffer).resize(400, null, { withoutEnlargement: true }).jpeg({ quality: 75 }).toBuffer(),
        suffix: 'small',
        width: 400
      });
      break;
...
async function checkUploadQuota(userId: string, type: string, size?: number): Promise<{allowed: boolean, message?: string}> {
  try {
    const quotaDoc = await db.collection('uploadQuotas').doc(userId).get();
    ...
    const newMonthlyUsed = (quotaData as any).monthlyUsed + (size || 0);
    if (newMonthlyUsed > (quotaData as any).monthlyLimit) {
      return {
        allowed: false,
        message: `Monthly upload limit exceeded. Used: ${Math.round(newMonthlyUsed / 1024 / 1024)}MB, Limit: ${Math.round((quotaData as any).monthlyLimit / 1024 / 1024)}MB`
      };
    }

    // Check daily upload count
    if ((quotaData as any).dailyUploads >= (quotaData as any).dailyLimit) {
      return {
        allowed: false,
        message: `Daily upload limit exceeded. Uploads today: ${(quotaData as any).dailyUploads}, Limit: ${(quotaData as any).dailyLimit}`
      };
    }
...
async function updateUploadQuota(userId: string, type: string, size: number): Promise<void> {
  try {
    const quotaRef = db.collection('uploadQuotas').doc(userId);
    
    await quotaRef.set({
      userId,
      monthlyUsed: FieldValue.increment(size),
