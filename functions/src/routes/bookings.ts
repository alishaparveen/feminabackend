...
const addConsultationNotesSchema = z.object({
 ...
 */
router.post('/', validateRequest(createBookingSchema), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.uid;
 ...
 */
router.get('/:bookingId', async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
router.put('/:bookingId/status', validateRequest(updateBookingStatusSchema), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { bookingId } = req.params;
    const userId = req.user!.uid;
    ...
    if (status === 'cancelled') {
      ...
      await db.collection('experts').doc(booking.expertId).update({
        completedConsultations: FieldValue.increment(1),
        totalEarnings: FieldValue.increment(booking.price / 100)
      });
 ...
 */
router.post('/:bookingId/reschedule', validateRequest(rescheduleBookingSchema), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { bookingId } = req.params;
    const userId = req.user!.uid;
    ...
    const hasConflict = conflictingBookings.docs.some(doc => {
      if (doc.id === bookingId) return false; // Ignore current booking
 ...
 */
router.post('/:bookingId/notes', expertOnly, validateRequest(addConsultationNotesSchema), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { bookingId } = req.params;
    const userId = req.user!.uid;
 ...
 */
router.post('/:bookingId/review', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { bookingId } = req.params;
    const userId = req.user!.uid;
 ...
 */
router.get('/:bookingId/meeting-link', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { bookingId } = req.params;
    const userId = req.user!.uid;
...
router.get('/upcoming', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const clientBookings = await Promise.all(
      clientBookingsQuery.docs.map(async (doc) => {
        const booking = doc.data();
        const expertDoc = await db.collection('experts').doc(booking.expertId).get();
        const expertData = expertDoc.data();
    ...
    const expertBookings = await Promise.all(
      expertBookingsQuery.docs.map(async (doc) => {
        const booking = doc.data();
        const clientDoc = await db.collection('users').doc(booking.userId).get();
        const clientData = clientDoc.data();
    ...
    const allBookings = [...clientBookings, ...expertBookings]
      .sort((a, b) => (a as any).scheduledAt.toDate().getTime() - (b as any).scheduledAt.toDate().getTime())
