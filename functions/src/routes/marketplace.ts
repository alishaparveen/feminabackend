...
const createProductSchema = z.object({
  ...
  category: z.enum(['beauty', 'health', 'fitness', 'fashion', 'books', 'courses', 'supplements', 'accessories']),
  tags: z.array(z.string()).max(10).optional(),
  images: z.array(z.string().url()).max(10).optional(),
  inventory: z.number().min(0).optional(),
  shippingRequired: z.boolean().default(true),
  digitalProduct: z.boolean().default(false),
...
router.get('/products', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (search) {
      ...
      products = products.filter(product => {
        ...
        return (
          ...
          (Array.isArray(tags) && tags.some((tag: string) => tag.toLowerCase().includes(searchTerm)))
        );
      });
    }
    ...
    const sellerIds = [
      ...new Set(
        products
          .map((product: any) => product.sellerId)
    ...
    const enrichedProducts = products.map(product => ({
      ...product,
      seller: sellersMap.get((product as any).sellerId) || null,
 ...
 */
router.post('/products', validateRequest(createProductSchema), async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
router.get('/products/:productId', async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
router.put('/products/:productId', validateRequest(updateProductSchema), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { productId } = req.params;
    const userId = req.user!.uid;
 ...
 */
router.delete('/products/:productId', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { productId } = req.params;
    const userId = req.user!.uid;
 ...
 * Create a new order
 */
router.post('/orders', validateRequest(createOrderSchema), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.uid;
 ...
 */
router.get('/orders/my-orders', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.uid;
    const { 
      role = 'buyer', 
 ...
 */
router.put('/orders/:orderId/status', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { orderId } = req.params;
    const userId = req.user!.uid;
    ...
    if (status === 'delivered') {
      await db.collection('products').doc(order.productId).update({
        soldCount: FieldValue.increment(order.quantity),
        inventory: order.inventory !== undefined ? 
          FieldValue.increment(-order.quantity) : undefined
      });
    }
 ...
 */
router.post('/products/:productId/review', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { productId } = req.params;
    const userId = req.user!.uid;
