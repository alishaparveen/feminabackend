...
const createResourceSchema = z.object({
  ...
  category: z.enum(['health', 'career', 'relationships', 'parenting', 'fitness', 'mental_health', 'nutrition', 'lifestyle']),
  type: z.enum(['article', 'guide', 'video', 'podcast', 'infographic', 'checklist']),
  ...
  sources: z.array(z.object({
    title: z.string(),
    url: z.string().url(),
    type: z.enum(['research', 'article', 'book', 'website'])
  })).optional()
});
  ...
  try {
    ...
    if (search) {
      ...
      resources = resources.filter(resource => {
        ...
        const description = (resource as any).description || '';
        const tags = (resource as any).tags || [];
        return (
    ...
    const authorIds = [
      ...new Set(
        resources
          .map((resource: any) => resource.authorId)
    ...
    const enrichedResources = resources.map(resource => ({
      ...resource,
      author: authorsMap.get((resource as any).authorId) || null
    }));
 ...
 */
router.post('/', expertOnly, validateRequest(createResourceSchema), async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
router.get('/:resourceId', async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
router.put('/:resourceId', validateRequest(updateResourceSchema), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { resourceId } = req.params;
    const userId = req.user!.uid;
 ...
 */
router.post('/:resourceId/bookmark', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { resourceId } = req.params;
    const userId = req.user!.uid;
 ...
 */
router.get('/bookmarked', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.uid;
 ...
 * Rate a resource
 */
router.post('/:resourceId/rating', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { resourceId } = req.params;
    const userId = req.user!.uid;
